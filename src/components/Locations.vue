<template>
  <div class="w-full flex flex-wrap mb-10 xl:mb-20">
    <div class="lg:w-5/12">
      <h1
      ...
      <div class="w-full flex flex-wrap text-center">
        ...
        <div class="w-full flex flex-wrap justify-center lg:justify-end">
          ...
          >
            <h1 class="syne-font text-2xl md:text-sm xl:text-2xl">Sucursal San Benito</h1>
            <h3 class="text-lg md:text-xs xl:text-lg">Abierto de 12:00 m.d. - 9:00 p.m.</h3>
            <h3 class="text-lg md:text-xs xl:text-lg">Calle la Reforma #543, Colonia San Benito</h3>
          </div>-->
          <div
            v-for="item in $store.getters.locations"
            :key="item.id"
            class="text-left w-10/12 mr-4 mt-3 py-2 pl-4 xl:pl-5 pr-24 xl:pr-64 rounded border border-black"
          >
            <h1 class="syne-font text-2xl md:text-sm xl:text-2xl">{{item.name}}</h1>
            <h3
              class="text-lg md:text-xs xl:text-lg"
            >{{ tConvert(item.opening_time) + " - " +  tConvert(item.closing_time) }}</h3>
            <h3 class="text-lg md:text-xs xl:text-lg">{{item.address}}</h3>
          </div>
        </div>
      </div>
    </div>
      ...
      axios
        .get(
          `https://api.elaniin.dev/api/locations?type=${
            this.isPickUpActive ? "takeaway" : "delivery"
          }`
        )
        .then((response) => {
          this.$store.commit("fetchLocations", response.data.data)
        })
        .catch((e) => {
          this.errors.push(e)
        })
